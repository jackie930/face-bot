...
def search_main(
 ...
 function: save json_files back to s3 (key: file name value: tag label)
    ...
    #load face
    eprint ("<<<<< load face search map")
    u = AnnoyIndex(512, 'euclidean')
    u.load('./dependency/test.ann')
    eprint ('<<<<<start match!')
    matches=u.get_nns_by_vector(v,1)
    eprint ("<<<< match success!")
    eprint ("matches", matches)

    file=open(name_list, 'r')
    list_read = file.readlines()
    match_name = list_read[matches[0]]
    eprint ("<<<<< Match Name:")
    eprint ("<<<<<", match_name)
    return match_name
...
if __name__ == "__main__":
    ...
    file_path ='to_compare.jpg'
    name_list = './name.txt'
    search_main(name_list,file_path, endpoint_name, region_name)
    eprint("<<< Exit.")